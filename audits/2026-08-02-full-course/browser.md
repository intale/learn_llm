...
| Every failed diagnostic test retried sequentially at four workers | 7 passed, 5 failed | 19 passed, 8 failed |
| Five navigation tests after their stale expectations were corrected in staging | 5 passed | 5 passed |

The failures come from three independent kinds of test drift:
...
repository's default 16 workers per engine. They added seven Chromium and
nineteen Firefox 30-second timeouts through CPU contention. Playwright's exact
failed-test sets were then rerun one engine at a time with four workers; every
timeout passed. The remaining failures were precisely the five stale navigation
expectations in both engines and the three Firefox geometry defects below.

This is a validation-system defect rather than evidence that 332 course pages
are broken. It is high severity because the normal gate is overwhelmingly red,
its earliest assertion masks real Russian-only behavior, and a future product
regression cannot be distinguished from known test drift without manual
diagnosis.
...
- A second audit-only sweep loaded all 85 routes with JavaScript disabled at
  ...
  passed in Firefox: pages remained within the viewport, static figures kept
  their semantic caption/style contract, full-view controls stayed absent,
