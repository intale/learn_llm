...
pub struct HistoricalCacheContrast {
    pub causal_decoder_stack: bool,
    pub explicit_previous_key_values: bool,
    pub prompt_then_decode_phases: bool,
    pub paging_is_deferred: bool,
}

// region:historical-cache-contrast
/// Summarizes the LLM-inference progression supported by the chapter's sources.
pub const fn historical_cache_contrast() -> HistoricalCacheContrast {
    HistoricalCacheContrast {
        causal_decoder_stack: true,
        explicit_previous_key_values: true,
        prompt_then_decode_phases: true,
        paging_is_deferred: true,
    }
}
...
fn complete_prefix_logits(model: &DecoderModel, prefix: &[u32]) -> Result<Vec<f64>, FixtureError> {
    let logits = no_grad(|| model.forward(prefix, &[1, prefix.len()]))?
        .logits()
        .value();
    let vocabulary_size = model.config().vocabulary_size();
    Ok(logits.as_slice()[logits.len() - vocabulary_size..].to_vec())
}
...
fn phase_evidence(
    ...
    let complete_prefix_logits = complete_prefix_logits(model, prefix)?;
...
fn error_evidence(model: &DecoderModel) -> Result<ErrorEvidence, FixtureError> {
    ...
    let decode_before_prefill_rejected = fresh.decode(model, 0).is_err() && fresh == fresh_before;

    fresh.prefill(model, &PROMPT)?;
    let populated = fresh.clone();
    let prefill_nonempty_rejected = fresh.prefill(model, &[0]).is_err() && fresh == populated;

    let rebuilt = DecoderModel::from_parameters(model.config(), copied_parameters(model))?;
    let rebuilt_model_rejected = fresh.decode(&rebuilt, 2).is_err() && fresh == populated;
    let changed = DecoderModel::from_parameters(fixture_config(2e-6), model.parameters().to_vec())?;
    let changed_config_rejected = fresh.decode(&changed, 2).is_err() && fresh == populated;

    fresh.decode(model, 2)?;
    fresh.decode(model, 3)?;
    let full = fresh.clone();
    let overflow_rejected = fresh.decode(model, 4).is_err() && fresh == full;
...
/// Proves model-wide cache coherence, loaded generation parity, reset, and errors.
pub fn learner_evidence() -> Result<LearnerEvidence, FixtureError> {
    ...
    let prefill = phase_evidence(&model, &cache, &prefill_output, &PROMPT, 0)?;
    let decode_output = cache.decode(&model, DECODE_TOKEN)?;
    let decode_prefix = [PROMPT[0], PROMPT[1], DECODE_TOKEN];
    let decode = phase_evidence(&model, &cache, &decode_output, &decode_prefix, PROMPT.len())?;
    ...
    let complete_prefix_attention_score_values =
        complete_prefix_attention_score_values(config, &[prefill.cache_after, decode.cache_after])?;
...
pub fn learner_report() -> Result<String, FixtureError> {
    ...
    writeln!(
        report,
        "prefill=prompt:{} cache:{}->{} layer_lengths:{} shape:{} max_abs_diff:{:.12} logits:{}",
        u32_vector(&PROMPT),
    ...
    writeln!(
        report,
        "decode=token:{DECODE_TOKEN} position:{} cache:{}->{} layer_lengths:{} shape:{} max_abs_diff:{:.12} logits:{}",
    ...
    writeln!(
        report,
        "loaded=checkpoint_bytes:{} rng_state:0x{:016x} prompt:{} generated:{} text:{} prefixes:{} stop:{} final_cache:{} cached_scores:{} complete_prefix_scores:{} tokens_match:{} rng_match:{}",
    ...
    writeln!(
        report,
        "history=causal_stack:{} previous_kv:{} prompt_decode:{} paging_deferred:{}",
        evidence.history.causal_decoder_stack,
        evidence.history.explicit_previous_key_values,
        evidence.history.prompt_then_decode_phases,
        evidence.history.paging_is_deferred,
    )
    .unwrap();
...
pub fn diagram_trace() -> Result<String, FixtureError> {
    ...
    writeln!(
        trace,
        "MATCH|phase=prefill|cached={}|complete_prefix={}|max_abs_diff={:.12}",
        fixed_vector(&evidence.prefill.cached_logits, 9),
    ...
    writeln!(
        trace,
        "MATCH|phase=decode|cached={}|complete_prefix={}|max_abs_diff={:.12}",
        fixed_vector(&evidence.decode.cached_logits, 9),
