...
The English-only example above matches the current Chapter 31–39 active sets;
Chapters 0–30 use English and Russian. A registered but inactive locale gets no
...
include a predict-first ordered list with answers in `<details>`. Prefer one
figure; register a supplementary `*Diagram` component only when it presents a
genuinely distinct relationship that would otherwise overload the primary figure.
