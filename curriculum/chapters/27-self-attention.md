---
{
  ...
  "worked_inputs": {
    "en": "Reuse Chapter 26's projected tensors $Q=[[[0,3],[2,-1]]]$, $K=[[[3,0],[-1,2]]]$, and $V=[[[3,-3],[1,3]]]$. Predict which value row each query will favor before running the fixture."
  },
  "formula": {
  ...
  "history": {
    "llm_evolution": {
      ...
      "limitation": {
        "en": "A basic recurrent encoder-decoder can compress an entire source sentence into one fixed-size vector, making long-range source details difficult to preserve; additive attention instead retrieves a new weighted source context for each decoder step but retains recurrent alignment steps between two sequence streams."
      },
      "later_advance": {
        "en": "The Transformer uses scaled dot-product attention and packs many queries into matrices; self-attention compares positions from the same sequence so one layer can form its full score grid together."
      },
      "modern_llm_role": {
        "en": "Each decoder self-attention head turns learned queries and keys into retrieval probabilities, then mixes learned values; a causal decoder additionally masks future key positions before normalization."
      },
      "sources": [
        {
          "role": "earlier",
        ...
        },
        {
          ...
          "claim": {
            "en": "Vaswani et al. define scaled dot-product attention as softmax-normalized scaled query-key products applied to values, describe packed simultaneous queries, and define self-attention as relating positions within one sequence."
          }
        }
      ]
    },
    ...
    "summary": {
      "en": "Additive attention replaced one fixed source summary with decoder-dependent retrieval from encoder annotations. Scaled dot-product self-attention makes every position in one sequence both a requester and a candidate source, forms a matrix of compatibilities, normalizes each query row, and mixes the value rows. This chapter leaves every position visible; Chapter 28 adds causal visibility."
    },
    "rust_contrast": "For the two-token fixture, record two recurrent decoding steps versus one two-by-two self-attention score grid. This exposes sequential alignment steps and all-position score structure without claiming constant total work, parallel autoregressive generation, or a universal runtime advantage."
  },
  "rust": {
  ...
  "translation_notes": [
    "Chapter 27 has the exact active locale set {en}. Russian is registered but inactive, so this contract intentionally has no ru keys and no Russian lesson or placeholder route.",
    "Keep Q, K, V, A, O, d_k, d_v, B, T, shapes, numeric values, error kinds, trace keywords, source roles, and source URLs unchanged when another locale is activated later.",
  ...
  "acceptance_examples": [
    {
      "input": "Apply unmasked attention to the frozen two-token Q, K, and V tensors",
    ...
    {
      "input": "Use batch shape [2,2,2], empty-batch shape [0,2,2], or value width 1",
    ...
    },
    {
      ...
      "expected": "The loss is -1.966576; query, key, and value gradients match the frozen report and all four coordinates of each tensor pass central differences within 0.000002."
    },
    {
...
The executable historical contrast reports two recurrent decoding steps and
four cells in this fixture's $2\times2$ self-attention score grid. Those counts
expose the structural change on the road to modern LLMs without pretending to be
a hardware benchmark or a programming-language comparison.
...
The primary reverse fixture seeds the four output coordinates with
$[1,0,0,1]$. Its loss is approximately $-1.966576$, and nonzero gradients reach
queries, keys, and values. Central differences check all four coordinates of
each input with step $10^{-6}$ and tolerance $2\times10^{-6}$. Tests also cover
...
1. Compute the four raw query-key dot products for the frozen fixture.
...
9. State what the two-step versus four-score historical count does and does not claim.
...
and the tiny counts contrast recurrent steps with score-grid structure without
proving constant work or end-to-end generation parallelism.
...
Future translation must preserve formula symbols, axes, shapes, numeric values,
trace tokens, error kinds, source roles, and source URLs.
