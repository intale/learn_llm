---
{
  ...
  "translation_notes": [
    ...
    "The English and Russian Chapter 2 cheat sheets must preserve the same ten-term source order: the existing nine entries remain unchanged and ordered, followed by one Byte-pair encoding (BPE) entry at Chapter 2's training-data boundary. Record their exact reviewed hashes in the completion ledger. The Test-partition definition distinguishes the generic post-selection role, one-execution enforcement, and later-execution regression reuse; the BPE definition keeps pair counts training-only and treats every document boundary as an adjacency barrier."
  ],
  "acceptance_examples": [
    {
