...
function record(
  ...
  tokens.forEach((token, index) => {
    ...
    if (key !== expectedKeys[index]) {
      fail(
        `${prefix.trim()} field ${index + 1} must be ${expectedKeys[index]}`,
      );
    }
...
function integer(value: string, field: string): IntegerLexeme {
  if (!integerPattern.test(value))
    fail(`${field} must be a canonical nonnegative integer`);
  const numeric = Number(value);
  if (!Number.isSafeInteger(numeric))
    fail(`${field} must be a safe nonnegative integer`);
  return { lexeme: value };
}

function decimal(value: string, field: string): DecimalLexeme {
  if (!decimalPattern.test(value))
    fail(`${field} must use canonical twelve-decimal notation`);
...
function expectVector(
  ...
  value.forEach((entry, index) => {
    if (entry.lexeme !== expected[index])
      fail(`${field} ${index} differs from Rust fixture`);
  });
}

export function parseSwigluFeedForwardTrace(
  source: string,
): SwigluFeedForwardTrace {
  ...
  if (lines[0] !== "TRACE swiglu-feed-forward-v1 BEGIN")
    fail("line 1 must be versioned BEGIN");
  if (lines[10] !== "TRACE swiglu-feed-forward-v1 END")
    fail("line 11 must be versioned END");
  ...
  const modelWidth = integer(
    fixtureRecord["model-width"],
    "FIXTURE model width",
  );
  const hiddenWidth = integer(
    fixtureRecord["hidden-width"],
    "FIXTURE hidden width",
  );
  const outputWidth = integer(
    fixtureRecord["output-width"],
    "FIXTURE output width",
  );
  const parameterCount = integer(
    fixtureRecord["parameter-count"],
  ...
  const forward = [0, 1].map((position) => {
    ...
    const parsed = {
      ...
      gatePre: vector(
        fields["gate-pre"],
        3,
        `POSITION-FORWARD ${position} gate pre`,
      ),
      gateSilu: vector(
        fields["gate-silu"],
        3,
        `POSITION-FORWARD ${position} gate SiLU`,
      ),
    ...
    expectVector(
      parsed.input,
      expected.input,
      `POSITION-FORWARD ${position} input`,
    );
    expectVector(
      parsed.gatePre,
      expected.gatePre,
    ...
    expectVector(
      parsed.gated,
      expected.gated,
      `POSITION-FORWARD ${position} gated`,
    );
    expectVector(
      parsed.output,
      expected.output,
      `POSITION-FORWARD ${position} output`,
    );
    return parsed;
  });
  ...
  const backward = [0, 1].map((position) => {
    ...
    const parsed = {
      ...
      upstream: vector(
        fields.upstream,
        2,
        `POSITION-BACKWARD ${position} upstream`,
      ),
      ...
      upGradient: vector(
        fields["up-gradient"],
        3,
        `POSITION-BACKWARD ${position} up gradient`,
      ),
  ...
  const parameterGradients = [0, 1, 2].map((index) => {
    const fields = record(lines[6 + index], "PARAMETER-GRADIENT ", [
      "name",
      "shape",
      "values",
    ]);
    ...
    expectText(
      fields.shape,
      expected.shape,
      `PARAMETER-GRADIENT ${index} shape`,
    );
    const values = vector(
      fields.values,
      6,
      `PARAMETER-GRADIENT ${index} values`,
    );
    ...
    return {
      ...
      shape:
        fields.shape as SwigluFeedForwardTrace["parameterGradients"][number]["shape"],
      values,
    };
  });
  ...
  const independence = {
    changedPosition: integer(
      independenceRecord["changed-position"],
      "INDEPENDENCE changed position",
    ),
    replacementInput: vector(
      independenceRecord["replacement-input"],
      2,
  ...
  expectInteger(
    independence.observedPosition,
    "1",
    "INDEPENDENCE observed position",
  );
  expectVector(
    independence.before,
    expectedForward[1].output,
    "INDEPENDENCE before",
  );
  expectVector(
    independence.after,
...
function label(
  ...
  if (typeof value !== "string" || value.trim() === "")
    fail(`${path}.${key} must be nonblank`);
}
...
export function validateSwigluFeedForwardLabels(
  ...
  labelGroup(
    labels.stages,
    ["positions", "gate", "up", "merge", "down", "independence", "gradients"],
    "labels.stages",
  );
  labelGroup(
    labels.fields,
  ...
  labelGroup(
    labels.notes,
    ["positionWise", "gate", "gradients", "independence", "precision"],
    "labels.notes",
  );
  labelGroup(labels.symbols, ["biasFree", "unchanged"], "labels.symbols");
  labelGroup(
    labels.captions,
    ["positionGradients", "parameterGradients"],
    "labels.captions",
  );
  labelGroup(labels.scrollers, ["forward", "parameters"], "labels.scrollers");
}
