---
{
  ...
  "worked_inputs": "Use cat as the ASCII input and кот as the paired Cyrillic input.",
  "formula": {
  ...
  "history": {
    ...
    "summary": "Early pipelines often split on whitespace into whole words; character-level models instead used much smaller symbol inventories. Whole-word vocabularies make unseen forms unknown, while scalar-level sequences are longer and still do not represent user-perceived grapheme clusters.",
    "rust_source": "rust/demos/ch01-text-units/src/lib.rs"
  },
  "decoder_connection": "The resulting Vec<usize> is the discrete sequence that will index embedding rows and enter the decoder; generated IDs travel through the inverse vocabulary mapping to become text.",
  "rust_sources": [
    {
...
> **Key idea:** UTF-8 byte length, Unicode-scalar length, and vocabulary length are different measurements; this chapter deliberately preserves only known scalar values.
...
Chapter 3 learns BPE merge rules over UTF-8 bytes, and Chapter 4 applies those
frozen rules to byte sequences while preserving this integer-sequence interface.
Everything that follows—
embeddings, attention, and next-token prediction—can therefore operate on IDs
without needing to know how many UTF-8 bytes produced each one.
