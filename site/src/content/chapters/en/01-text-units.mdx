---
{
  ...
  "description": "Compare UTF-8 bytes, Unicode scalar values, and a demo-only scalar vocabulary before byte-level BPE replaces it.",
  ...
  "rust_sources": [
    ...
    },
    {
      "path": "rust/demos/ch01-text-units/src/lib.rs",
      "region": "vocabulary",
      "purpose": "Implement the demo-only scalar vocabulary's deterministic construction, encoding, and decoding."
    },
    {
...
cumulative model. In that model, each ID selects one row from an embedding table
before the sequence enters the decoder. The decoder eventually predicts another
ID, and the inverse mapping for that same vocabulary turns generated IDs back into
text.
