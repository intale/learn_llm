...
const componentSource = read(
  "site/src/components/chapters/SwigluFeedForwardDiagram.astro",
);
...
const labels: SwigluFeedForwardDiagramLabels = {
  ...
  scrollers: { forward: "forward scroller", parameters: "parameter scroller" },
};
...
describe("Chapter 20 Rust trace parser", () => {
  it("preserves exact branch, reverse, parameter, and independence evidence", () => {
    ...
    expect(
      trace.forward.map((row) => row.output.map(({ lexeme }) => lexeme)),
    ).toEqual([
    ...
    expect(
      trace.backward.map((row) =>
        row.inputGradient.map(({ lexeme }) => lexeme),
      ),
    ).toEqual([
    ...
    expect(
      trace.parameterGradients.map(({ name, shape }) => [name, shape]),
    ).toEqual([
      ["ffn.gate.weight", "2x3"],
      ["ffn.up.weight", "2x3"],
      ["ffn.down.weight", "3x2"],
    ...
    expect(
      trace.parameterGradients[2].values.map(({ lexeme }) => lexeme),
    ).toEqual([
      "-0.268941421370",
      "0.000000000000",
      "0.000000000000",
  ...
  it.each([
    ...
    [
      "missing record",
      fixture.replace(/^PARAMETER-GRADIENT name=ffn\.up.*\n/m, ""),
      /exactly 11 lines/,
    ],
    [
      "version drift",
      fixture.replace("swiglu-feed-forward-v1", "swiglu-feed-forward-v2"),
      /line 1/,
    ],
    [
      "fixture name",
      fixture.replace("name=known-position-wise-swiglu", "name=other"),
      /FIXTURE name/,
    ],
    [
      "unsafe width",
      fixture.replace("model-width=2", "model-width=99999999999999999999"),
      /safe nonnegative integer/,
    ],
    [
      "parameter count",
      fixture.replace("parameter-count=18", "parameter-count=17"),
      /FIXTURE parameter count/,
    ],
    [
      "output width",
      fixture.replace("output-width=2", "output-width=3"),
      /FIXTURE output width/,
    ],
    ["bias policy", fixture.replace("bias=false", "bias=true"), /FIXTURE bias/],
    [
      "input shape",
      fixture.replace("input-shape=2x2", "input-shape=1x2x2"),
      /FIXTURE input shape/,
    ],
    [
      "forward position",
      fixture.replace(
        ...
        "POSITION-FORWARD position=9",
      ),
      /POSITION-FORWARD 1 position/,
    ],
    [
      "forward value",
      fixture.replace("output=1.924234314520", "output=1.924234314521"),
      /POSITION-FORWARD 0 output 0/,
    ],
    [
      "negative zero",
      fixture.replace("0.000000000000", "-0.000000000000"),
      /negative zero/,
    ],
    [
      "short vector",
    ...
    [
      "backward value",
      fixture.replace(
        "input-gradient=4.634916362006",
        "input-gradient=4.634916362007",
      ),
      /POSITION-BACKWARD 0 input gradient 0/,
    ],
    [
      "parameter name",
      fixture.replace("name=ffn.up.weight", "name=ffn.other.weight"),
      /PARAMETER-GRADIENT 1 name/,
    ],
    [
      "parameter shape",
      fixture.replace(
        "name=ffn.down.weight shape=3x2",
        "name=ffn.down.weight shape=2x3",
      ),
      /PARAMETER-GRADIENT 2 shape/,
    ],
    [
      "independence result",
      fixture.replace("unchanged=true", "unchanged=false"),
      /INDEPENDENCE unchanged/,
    ],
    [
      "independence drift",
      fixture.replace("after=-0.268941421370", "after=-0.268941421371"),
      /INDEPENDENCE after 0/,
    ],
    [
      "field order",
      fixture.replace(
        "model-width=2 hidden-width=3",
        "hidden-width=3 model-width=2",
      ),
      /FIXTURE field 2/,
    ],
    [
      "extra field",
      fixture.replace(
        "bias=false parameter-count=18",
        "bias=false extra=yes parameter-count=18",
      ),
...
describe("Chapter 20 labels and static component", () => {
  it("accepts the complete label tree and rejects blank or missing leaves", () => {
    ...
    expect(() =>
      validateSwigluFeedForwardLabels({
        ...labels,
        scrollers: { ...labels.scrollers, parameters: "" },
      }),
    ).toThrow(/labels\.scrollers\.parameters/);
    expect(() =>
      validateSwigluFeedForwardLabels({} as SwigluFeedForwardDiagramLabels),
  ...
  it("projects Rust-owned evidence and server-renders every diagram equation", () => {
    ...
    expect(componentSource).toContain(
      "String.raw`u_{${row.position.lexeme}}=X_{${row.position.lexeme}}W_u`",
    );
    expect(componentSource).toContain(
      'latex="h=s\\odot u"',
    );
    ...
    expect(componentSource).toContain(
      "String.raw`Y_{${row.position.lexeme}}=${latexVector(row.output)}`",
    );
    expect(componentSource).toContain(
      "String.raw`dX_{${row.position.lexeme}}`",
    );
    expect(componentSource).toContain("String.raw`\\sum_p X_p^\\top dA_p`");
    expect(componentSource).toContain("String.raw`\\sum_p X_p^\\top dU_p`");
    expect(componentSource).toContain("String.raw`\\sum_p H_p^\\top G_p`");
    expect(componentSource).toContain(".toFixed(6)");
    expect(componentSource).not.toMatch(/<script|client:/);
    expect(componentSource).not.toContain("<svg");
    expect(parserSource).not.toMatch(
      /Math\.|random\(|reduce\(|sqrt\(|pow\(|parseFloat\(/,
    );
  });

  it("uses the shared diagram contract and keeps only concept geometry locally", () => {
    expect(componentSource).toContain('class="course-diagram swiglu-diagram"');
    ...
    expect(
      componentSource.match(/data-diagram-box/g)?.length ?? 0,
    ).toBeGreaterThanOrEqual(16);
    expect(componentSource).toContain(
      'data-diagram-table class="forward-table"',
    );
    expect(componentSource).toContain('data-forward-stage="gate-activation"');
    expect(componentSource).toContain("min-inline-size: 36.25rem");
    expect(componentSource).toContain(
      "@container course-diagram (max-width: 36rem)",
    );
    expect(componentSource).not.toContain(
      "@container course-diagram (min-width:",
    );
    expect(componentSource).toContain(".swiglu-diagram:fullscreen");
    expect(componentSource).toContain(
    ...
    expect(componentSource).not.toMatch(
      /\.forward-table\s*\{[^}]*(?:height|min-height|block-size)\s*:/s,
    );
    expect(componentSource).not.toMatch(
    ...
    expect(componentSource).not.toMatch(/overflow(?:-x|-y)?\s*:/);
...
describe("Chapter 20 contract and lesson projection", () => {
    ...
    expect(lesson).toMatchObject({
      ...
      formula: {
        ...
        symbols: contract.formula.symbols.map(
          (symbol: { symbol: string; en: string }) => ({
  ...
  it("orders the pedagogy and renders all explanatory equations as math", () => {
    ...
    for (const formula of [
      ...
      expect(lessonBody.replace(/\s+/g, "")).toContain(
        formula.replace(/\s+/g, ""),
      );
    }
    ...
    expect(lessonBody).toContain(
      "road from nonlinear neural-language-model computation to modern LLM",
    );
  ...
  it("codifies formula rendering while retaining code formatting for program artifacts", () => {
    ...
    expect(lessonBody).toContain(
      "`cargo run --quiet --locked -p ch20-swiglu-feed-forward`",
    );
    expect(lessonBody).not.toContain("build-time parser");
    expect(lessonBody).not.toContain("keyboard-focusable scroller");
    expect(lessonBody).not.toContain(
      "`rust/demos/ch20-swiglu-feed-forward/expected.txt`",
    );
  });
});
