...
const copy = {
  ...
  ru: {
    ...
    chapterTitle: 'Применение и обращение токенизатора BPE',
    revisionLabel: 'Версия материала',
    headings: {
      formula: 'Гарантируйте точные байты в одном направлении',
      history: 'Решите проблему неизвестных строк с помощью байтового покрытия',
      rust: 'Соберите зафиксированный строгий токенизатор на Rust',
      visualization: 'Проследите группировку и точную обратную конкатенацию',
      exercises: 'Сначала предскажите, затем проверьте',
      decoder: 'Сохраняйте отдельную последовательность с границами для каждого документа',
    },
    rustCaptions: [
      'Закрытая таблица слов, которая не может восстановить невиданное написание',
      'Версия 1 схемы токенизатора и проверяемые диапазоны ID',
      'Преобразуйте байты в ID и примените все зафиксированные ранги',
      'Сначала восстановите байты, а текст проверяйте только по запросу',
      'Добавьте управляющие токены по краям после кодирования и строго проверьте их',
      'Выведите точные свидетельства токенизатора, граничных случаев и визуализации',
    ],
    diagramTitle: 'Группы байтов по рангам точно восстанавливают вход',
    cases: [
      'Пример ASCII: bee и пробел',
      'Кириллический пример: пробел и а',
    ],
    grouped: 'Канонические группы по рангам',
    exact: 'Точное совпадение байтов',
    invariants: [
      'Зафиксированные ранги применяются по возрастанию; вход не меняет их приоритет.',
      'Каждый ID содержимого равен ID из пространства обучения главы 3 плюс два.',
      'BOS и EOS добавляются после кодирования и находятся только на краях документа.',
      'Сохранённые байты частей складываются в точный вход без нормализации.',
    ],
