...
const formerChapter14To32MathCode = new Set([
  "square=4",
  "loss=8",
  "bar(loss)=1",
  "bar(square)=2",
...
const chapter14To32Latex: Record<
  (typeof chapter14To32Ids)[number],
  readonly string[]
> = {
...
test.describe("@formula-rendering:ch14-ch32 rendered formula contract", () => {
  for (const [viewportName, viewport] of Object.entries(viewports)) {
    for (const chapterId of chapter14To32Ids) {
      test(`${viewportName} en/${chapterId} exposes readable server-rendered math`, async ({
        page,
      }) => {
        await page.setViewportSize(viewport);
        ...
        const latex = await page
          ...
          .evaluateAll((nodes) => nodes.map((node) => node.textContent ?? ""));
        for (const fragment of chapter14To32Latex[chapterId]) {
          expect(
            latex.some((expression) => expression.includes(fragment)),
            `${chapterId} should render ${fragment}`,
          ).toBe(true);
        ...
        expect(
          inlineCode.filter((value) =>
            formerChapter14To32MathCode.has(value.trim()),
          ),
        ).toEqual([]);
      });
    }
