      ...
      nodes.flatMap((node, index) => {
        ...
        for (const [strutIndex, strut] of Array.from(struts).entries()) {
          if (getComputedStyle(strut).display !== 'inline-block') {
            issues.push(source + ' strut ' + strutIndex + ' is not inline-block');
          }
        }
...
async function expectChapterContent(
  ...
  const feedForwardTokenOne = await diagram
    ...
    .allTextContents();
  expect(feedForwardTokenOne).toContain('[0.010896,7.512278,0.000000,0.000000]');
  ...
  await expect(details).toContainText('The feed-forward branch can still change');
...
test.describe(
  ...
  () => {
    ...
    test('RTL prose keeps formulas, trace values, and causal table order left-to-right', async ({
      ...
      expect(
        await diagram.locator('.technical, [data-inline-math]').evaluateAll(
          (nodes) =>
            nodes.every((node) => getComputedStyle(node).direction === 'ltr'),
        ),
      ).toBe(true);
      expect(
        await diagram.locator('[data-attention-head]').evaluateAll((rows) =>
          rows.map((row) => [
