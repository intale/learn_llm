...
const copy: Record<ChapterLocale, LocalizedCopy> = {
  ...
  ru: {
    ...
    title: 'Выполняйте обратный проход через тензорные операции с локальными VJP',
    description:
      'Постройте на Rust ленту автоматического дифференцирования тензоров, проводите обратный проход через преобразования формы, согласование форм и редукции с помощью произведений вектора на якобиан (VJP) для отдельных рёбер и проверяйте градиенты для обучения LLM.',
    headings: [
      ...
      'Владейте прямыми значениями тензоров и сохраняйте локальный контекст',
    ...
    diagramDescription:
      'Проследите восемь узлов прямого прохода, восемь упорядоченных рёбер операндов, начальную сопряжённую величину нескалярного выхода, VJP с точными формами, накопление только в параметрах, освобождение графа, detach, численные проверки и безопасный отказ ошибочных запросов.',
    diagramSections: [
      'Постройте один граф с изменениями формы',
      'Проведите начальную сопряжённую величину по каждому ребру',
      'Восстановите точные формы параметров',
      'Сохраняйте, накапливайте, обнуляйте, освобождайте',
      'Проверьте сумму, detach и каждый VJP',
      'Отклоняйте небезопасные запросы без изменений',
    ],
    diagramTerms: [
