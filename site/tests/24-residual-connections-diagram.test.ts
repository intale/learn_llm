...
describe('Chapter 24 Rust trace parser', () => {
  it('preserves every exact forward, reverse, shape, stack, and proof record', () => {
    ...
    expect(trace.zeroBranch.weightGradientNonzero).toBe('true');
  ...
  it.each([
    ...
    ['site arithmetic', fixture.replace('site-arithmetic=none', 'site-arithmetic=gradients')],
...
describe('Chapter 24 static diagram boundary', () => {
  it('projects the Rust fixture without tensor arithmetic or client JavaScript', () => {
    ...
    expect(rustTraceSource).toContain('site-arithmetic=none');
    for (const rustOwnedField of [
      ...
      'trace.proof.siteArithmetic',
    ]) {
      expect(componentSource).toContain(rustOwnedField);
    }
  });

  it('uses semantic grids, local scrollers, natural heights, and non-color cues', () => {
    expect(componentSource).toContain('class="path-grid"');
  ...
  it('keeps metadata, exact output, formulas, history, and deferred locale aligned', () => {
    ...
    expect(lessonSource).toContain('The history is about neural architectures');
