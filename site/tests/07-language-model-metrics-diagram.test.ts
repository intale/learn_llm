...
const componentSource = readFileSync(
  resolve(
    process.cwd(),
    "src/components/chapters/LanguageModelMetricsDiagram.astro",
  ),
  "utf8",
);
const contractSource = readFileSync(
  resolve(repositoryRoot, "curriculum/chapters/07-language-model-metrics.md"),
  "utf8",
);
const englishLessonSource = readFileSync(
  resolve(
    repositoryRoot,
...
const metricSource = readFileSync(
  resolve(repositoryRoot, "rust/crates/llm-from-scratch/src/metrics.rs"),
  "utf8",
);
...
function mutate(search: string, replacement: string): string {
  ...
  if (
    first === -1 ||
    frozenTrace.indexOf(search, first + search.length) !== -1
  ) {
...
function stringLeafPaths(
  ...
  return Object.entries(value).flatMap(([key, child]) => {
    ...
    return typeof child === "string"
      ? [path]
      : stringLeafPaths(child as object, path);
  });
}
...
describe("language-model-metrics Rust trace parser", () => {
      ...
      scored: [
        {
          ...
          totalSurprise: {
            lexeme: "7067.943541648752",
            value: 7067.943541648752,
          },
        ...
        },
        {
          ...
          totalSurprise: {
            lexeme: "1867.529710185699",
            value: 1867.529710185699,
          },
  ...
  it.each([
    ...
    "rejects additional terminal LF records with exact typed semantics",
    ({ suffix, line }) => {
  ...
  it("rejects missing and trailing records before projecting shifted slots", () => {
    ...
    const withoutAggregate =
      [...records.slice(0, 4), ...records.slice(5)].join("\n") + "\n";
  ...
  it("is a locale-neutral projection with no metric or model implementation", () => {
    ...
    for (const lexeme of frozenMetricLexemes)
      expect(parserSource).not.toContain(lexeme);
  });
});
...
describe("language-model-metrics locale-owned labels", () => {
  it("requires every visible and accessible label leaf to be nonblank", () => {
    expect(() =>
      assertLanguageModelMetricsDiagramLabels(completeLabels),
    ).not.toThrow();
    ...
    for (const path of paths) {
      expect(() =>
        assertLanguageModelMetricsDiagramLabels(
          blankLabelAt(completeLabels, path),
        ),
      ).toThrow(path.join("."));
    }
  });
  ...
  it("fails closed for a missing group or a non-object group", () => {
    const missingSections = structuredClone(
      completeLabels,
    ...
    const invalidNotes = structuredClone(completeLabels) as unknown as Record<
      string,
      unknown
    >;
...
describe("Chapter 7 bounded test-evaluation handoff", () => {
  it("publishes revision 7 from one frozen English source with a direct Russian projection", () => {
    ...
    expect(english.content_revision).toBe(7);
    expect(russian.content_revision).toBe(7);
    expect(english.decoder_connection).toBe(contract.decoder_connection.en);
    expect(russian.decoder_connection).toBe(contract.decoder_connection.ru);
    expect(contract.translation_notes.join(" ")).toContain(
      createHash("sha256").update(englishLessonSource).digest("hex"),
    );
    expect(contract.translation_notes.join(" ")).toContain(
    ...
    expect(createHash("sha256").update(englishLessonSource).digest("hex")).toBe(
      "75b7e773ea37047b353f7e82d18b8da4ec8a7615cb981bbe9446c33a9a69d516",
    );
    expect(createHash("sha256").update(russianLessonSource).digest("hex")).toBe(
      "5ebb33ac1e32cf234c8eec1ce23f8d76f0726abada95a7bdd5a2ac4c7866a739",
    );
  });

  it("keeps Chapter 7 train/validation-only and narrows the Chapter 34 handoff", () => {
    const scoringRegion = metricSource.match(
      /\/\/ region:train-validation-scoring([\s\S]*?)\/\/ endregion:train-validation-scoring/,
    )?.[1];
    expect(scoringRegion).toBeDefined();
    expect(scoringRegion).toMatch(
      /pub enum ScoredPartition\s*\{\s*Train,\s*Validation,?\s*\}/,
    );
    const scoredPartitionBody = scoringRegion?.match(
      /pub enum ScoredPartition\s*\{([\s\S]*?)\}/,
    )?.[1];
    expect(scoredPartitionBody).toBeDefined();
    expect(scoredPartitionBody).not.toMatch(/\bTest\b/);
    expect(scoringRegion).toContain(
      "let _missing_variant: ScoredPartition = ScoredPartition::Test;",
    );

    const english = englishLessonSource.replace(/\s+/g, " ");
    const russian = russianLessonSource.replace(/\s+/g, " ");
    ...
    expect(russian).toContain(
      "В главе 34 позднее будет показана одна локальная оценка фиксированного учебного примера после завершения выбора модели",
    );
    expect(russian).toContain(
...
describe("language-model-metrics diagram component contract", () => {
  ...
  it("exposes frozen provenance, train then validation, and boundary evidence", () => {
    ...
    expect(componentSource).toContain(
      "trace.provenance.fittedDocuments.lexeme",
    );
    ...
    expect(componentSource).toContain(
      "data-scored-partition={score.partition}",
    );
    ...
    expect(componentSource).toContain(
      "test_selectable={trace.boundary.testSelectable}",
    );
  });
  ...
  it("keeps native score headers and uses one readable source-order full-view flow", () => {
    ...
    expect(rowHeaderRule).toMatch(
      /border-inline-start\s*:\s*0\.35rem solid currentColor/,
    );
    ...
    const calculationPosition = componentSource.indexOf(
      'class="calculation-panel"',
    );
    const frozenPosition = componentSource.indexOf(
    ...
    expect(causalPositions).toEqual(
      [...causalPositions].sort((left, right) => left - right),
    );
    ...
    const fullscreenStyles = componentSource.slice(
      fullscreenStart,
      fullscreenEnd,
    );
    ...
    expect(fullscreenStyles).not.toMatch(
      /transform\s*:\s*(?:scale|translate|skew)/,
    );
    expect(fullscreenStyles).not.toMatch(
      /overflow(?:-[xy])?\s*:\s*(?:hidden|clip)/,
    );
    expect(fullscreenStyles).not.toMatch(
      /contain\s*:\s*(?:paint|strict|content)/,
    );
  ...
  it("keeps causal progression and arrow direction coherent in RTL locales", () => {
    ...
    expect(stagePositions).toEqual(
      [...stagePositions].sort((left, right) => left - right),
    );
    ...
    expect(componentSource).not.toContain(
      '<div class="calculation-chain" dir="ltr">',
    );
    expect(componentSource).not.toContain(
      'class="metrics-diagram"\n  dir="ltr"',
    );
  });
