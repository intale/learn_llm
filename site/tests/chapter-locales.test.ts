...
describe('chapter-locale manifest', () => {
  it('keeps all authoring guides aligned with the current bilingual range', () => {
    ...
    expect(normalized('curriculum/README.md')).toContain(
      'activates English and Russian for every chapter from 0 through 39',
    );
    expect(normalized('curriculum/chapter-template.md')).toContain(
      'bilingual example above matches the current active set for Chapters 0–39',
    );
    expect(normalized('SKILLS.md')).toContain(
      'every chapter from 0 through 39 uses English and Russian',
    );
  });
  ...
  it('projects the canonical 40-chapter policy at all activation boundaries', () => {
    ...
    expect(chapterLocaleConfiguration.chapters[28]).toMatchObject({
      chapterId: '28-causal-masking',
      order: 28,
      activeLocales: ['en', 'ru'],
    });
    expect(chapterLocaleConfiguration.chapters[29]).toMatchObject({
      chapterId: '29-rope',
      order: 29,
      activeLocales: ['en', 'ru'],
    });
    expect(chapterLocaleConfiguration.chapters[31]).toMatchObject({
      chapterId: '31-decoder-block',
      order: 31,
      activeLocales: ['en', 'ru'],
    });
    expect(chapterLocaleConfiguration.chapters[32]).toMatchObject({
    ...
    expect(chapterLocaleConfiguration.chapters[36]).toMatchObject({
      chapterId: '36-temperature-top-k',
      order: 36,
      activeLocales: ['en', 'ru'],
    });
    expect(chapterLocaleConfiguration.chapters[37]).toMatchObject({
    ...
    expect(activeLocalesForChapter('28-causal-masking')).toEqual(['en', 'ru']);
    expect(isChapterLocaleActive('28-causal-masking', 'ru')).toBe(true);
    expect(activeLocalesForChapter('29-rope')).toEqual(['en', 'ru']);
    expect(isChapterLocaleActive('29-rope', 'ru')).toBe(true);
    expect(activeLocalesForChapter('30-multi-head-attention')).toEqual(['en', 'ru']);
    expect(isChapterLocaleActive('30-multi-head-attention', 'ru')).toBe(true);
    expect(activeLocalesForChapter('31-decoder-block')).toEqual(['en', 'ru']);
    ...
    expect(activeLocalesForChapter('36-temperature-top-k')).toEqual(['en', 'ru']);
    expect(isChapterLocaleActive('36-temperature-top-k', 'ru')).toBe(true);
    expect(activeLocalesForChapter('37-incremental-attention')).toEqual(['en', 'ru']);
    expect(isChapterLocaleActive('37-incremental-attention', 'ru')).toBe(true);
    expect(activeLocalesForChapter('38-cached-generation')).toEqual(['en', 'ru']);
    expect(isChapterLocaleActive('38-cached-generation', 'ru')).toBe(true);
    expect(activeLocalesForChapter('39-end-to-end-llm')).toEqual(['en', 'ru']);
    expect(isChapterLocaleActive('39-end-to-end-llm', 'ru')).toBe(true);
